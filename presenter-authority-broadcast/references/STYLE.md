...
domain: "News & Public Affairs"
identity: "A sober institutional broadcast separates decision, deadline, affected public, and implementation so authority comes from clarity rather than ornament."
style_master_a: "assets/style-master-a.png"
style_master_b: "assets/style-master-b.png"
recognition_anchors:
  - "one dominant institutional field"
  - "high-contrast decision surface"
  - "restrained authority accent"
  - "isolated deadline signal"
feel: "official, composed, direct, accountable"
palette_variants:
  authority-broadcast:
    background: "#16243B"
    foreground: "#FFFFFF"
...
A sober institutional broadcast separates decision, deadline, affected public, and implementation so authority comes from clarity rather than ornament.

Best suited to national updates, policy briefings, institutional news, public-service information. Avoid using it as a generic skin for consumer lifestyle, luxury campaigns, playful product demos. Domain logic must shape the composition, not merely the copy.

## Style Master fidelity

Treat `assets/style-master-a.png` and `assets/style-master-b.png` as the approved identity pair. A generated sequence should preserve at least three recognition anchors: one dominant institutional field, a high-contrast decision surface, a restrained authority accent, and an isolated deadline signal. Do not copy either frame mechanically, and do not claim fidelity from palette alone.
...
The forty content-form recipes in `LAYOUT-CATALOG.md` classify the relationship the viewer must understand. `HYPERFRAMES-LAYOUT-MAP.md` records a tested official Registry component or block for each form, and the 1920×1080 PNG gallery proves those items inside the Authority stage. The twelve composition archetypes provide deeper visual evidence for the surrounding style shell. None of these layers defines beats or required scene roles. Choose one semantic recipe first, inspect its official render proof, then borrow from at most two archetypes and install the strongest current Registry item for the real content.

## Color system

Seven named palettes live inside one Broadcast template. Layout hierarchy, component geometry, presenter grammar, and motion semantics stay identical. The user selects one name once, and bootstrap writes that project-level choice to the host and every scene:

- `authority-broadcast` (default here): navy field, white information, blue authority accent, amber signal.
...
Layout and scene CSS may consume only `--broadcast-field`, `--broadcast-ink`, `--broadcast-surface`, `--broadcast-surface-strong`, `--broadcast-line`, `--broadcast-muted`, `--broadcast-accent`, `--broadcast-signal`, `--broadcast-on-accent`, `--broadcast-on-signal`, `--broadcast-contrast-field`, `--broadcast-contrast-ink`, and `--broadcast-shadow`. Concrete color values belong only in `assets/runtime/color-system.css` or `custom-color-system.css`.
