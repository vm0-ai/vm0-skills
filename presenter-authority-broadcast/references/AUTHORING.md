# Authority Broadcast Authoring Exceptions
...
Built-in names are `authority-broadcast`, `monumental-minimal`, `black-gold`, `obsidian-champagne`, `petrol-brass`, `parchment-oxblood`, and `porcelain-carbon`. Use `--color-system custom --color-tokens <CSS_FILE>` for a user-supplied collection. This single project-level name is propagated to all scenes and recorded in `.style-reference/authority-broadcast/COLOR-SYSTEM.json`. The supplied CSS defines the complete `--broadcast-*` contract from `STYLE.md`; layout geometry is unchanged.

After content authoring, switch the whole project without scaffolding again:

```bash
node scripts/authority-set-color-system.mjs --project . --color-system black-gold
```

## Edit an executable starter

Each selected proof has a matching file under `.style-reference/authority-broadcast/layouts/source/`. The scaffold copies that executable contract into the real scene.
...
- `M` starters do the same with the supplied Authority adapter that combines the mapped official text behaviors.
...
```bash
node scripts/authority-review.mjs --project . --phase preflight
node scripts/authority-review.mjs --project . --phase static
node scripts/authority-review.mjs --project . --phase preview
node scripts/authority-review.mjs --project . --phase release
```

- `preflight` runs Authority contract, local-font, path, timing, and sidecar checks plus one HyperFrames lint command.
