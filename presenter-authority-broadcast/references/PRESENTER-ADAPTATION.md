...
Use this guide after Authority Broadcast is selected and before scene geometry is authored. It governs whether a presenter exists, how the person returns, and how content fits around the standard-size full or grounded head-and-shoulders treatment. A presenter may be a static transparent image or a real talking-avatar video; do not confuse the bundled PNG with a generated digital human. It does not create a scene schema or a layout renderer.
...
```html
<main class="authority-frame" data-presenter="off" data-title-chrome="off">
  <section class="authority-content-stage">...</section>
</main>
```
...
```html
<main class="authority-frame" data-presenter="on" data-presenter-layout="corner-right" data-title-chrome="off">
  <section class="authority-content-stage">...</section>
...
If the complete person weakens the composition, switch sides or use the grounded head-and-shoulders treatment. If that still fails, move the presenter to a divider scene or remove it from this scene. Do not shrink the full person or reduce critical copy below the Authority readability floor to preserve coexistence.
