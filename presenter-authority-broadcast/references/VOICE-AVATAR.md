...
`auto` creates a six-second authoring window marked `data-authority-timing="provisional"`; it is not a duration estimate. Use `--presenter on` when any talking-avatar scene exists. If scene IDs or layouts are not locked yet, use `--prepare-only` instead and delay scene creation. Never render provisional timing.
...
```bash
node <PROJECT_DIR>/scripts/authority-finalize-timing.mjs \
  --project <PROJECT_DIR> \
