# Authority Broadcast Fast Router

This file is the operational first stop for scene selection. Use the smallest row that truthfully holds the message. The mapped Registry item is the default first choice: install it directly. Search the Registry only when that item is unavailable, fails the real capacity, or expresses the wrong relationship; on that exception compare no more than two candidates total.

Codes: `C` = content-complete component merged into the scene; `B` = content-complete block mounted as a scene-isolated sub-composition; `M` = supplied Authority content adapter combining the named official behaviors. A motion primitive, token skeleton, or slot demo is never a finished layout by itself. Presenter guidance follows the global preference: standard-size `full`, then grounded bottom-corner `head-shoulders`, then divider or omission. Never shrink the full person.

| Layout ID | Choose when | Capacity | Default official item | Motion family | Presenter |
| --- | --- | --- | --- | --- | --- |
| `orientation/authority-cover` | One authoritative announcement | 4–12-word headline + 1–2 lines | C `titlecard-lockup` | text hierarchy | optional |
