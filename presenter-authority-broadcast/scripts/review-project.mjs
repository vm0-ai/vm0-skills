...
function projectTextFiles(projectRoot) {
  ...
  for (const relative of ["compositions", "assets/authority-broadcast"]) {
...
export function preflightProject(projectRoot, parsed) {
  ...
  const tokenCss = fs.existsSync(path.join(projectRoot, "assets/authority-broadcast/authority.css"))
    ? fs.readFileSync(path.join(projectRoot, "assets/authority-broadcast/authority.css"), "utf8")
    : "";
  ...
  for (const file of projectTextFiles(projectRoot)) {
    ...
    if (/@font-face\s*\{[^}]*font-family\s*:\s*["']?(?:AuthorityContent|[^;}]*CJK)/is.test(source)) nonLatinFontDeclared = true;
...
export function sourceFingerprint(projectRoot) {
  ...
  for (const relative of ["assets/authority-broadcast/fonts"]) {
...
export function fingerprintState(projectRoot, parsed) {
  ...
  for (const relative of ["assets/authority-broadcast", "assets/runtime/gsap.min.js"]) {
    const start = path.join(projectRoot, relative);
    if (!fs.existsSync(start)) continue;
...
function loadPreviewReport(projectRoot) {
  const file = path.join(projectRoot, ".hyperframes/authority-review/preview/report.json");
...
function buildSummary(report) {
  const lines = [
    `# Authority Broadcast ${report.phase} review`,
    "",
...
export function main(argv = process.argv.slice(2)) {
  ...
  const outputRoot = path.resolve(args.output || path.join(projectRoot, ".hyperframes", "authority-review", args.phase));
...
if (import.meta.url === invokedPath) {
  try {
    ...
    console.error(`Authority review failed: ${error.message}`);
    process.exitCode = 1;
  }
}
