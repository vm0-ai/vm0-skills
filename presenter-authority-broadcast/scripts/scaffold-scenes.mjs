...
function parseArgs(argv) {
  const args = { hostId: "authority-broadcast", language: "en", colorSystem: "authority-broadcast", presenterScenes: [], force: false };
...
function adaptStarter(source, sourceId, compositionId, sceneId, duration, language, colorSystem, presenter) {
  let adapted = source
    ...
    .replace(/id="authority-layout-stage"/, 'id="root"')
    .replace(/(<main id="root"[^>]*data-presenter=")(?:off|on)(")/, `$1${presenter ? "on" : "off"}$2`)
    .replace(/<main\s+class="authority-frame/, `<main id="scene-${sceneId}-stage" class="authority-frame`);
...
function bakeVariableDefaults(source, values, sceneId) {
  ...
  return baked.replace(htmlOpeningTag, opening => [
    `<!-- AUTHORITY_CONTENT_SLOT_BEGIN (${sceneId}): edit only default values in data-composition-variables. -->`,
    opening,
...
function markBlockContent(source, sceneId) {
  ...
  return source
    .replace(/(<body[^>]*>)/, `$1\n<!-- AUTHORITY_CONTENT_SLOT_BEGIN (${sceneId}): edit visible literals or data only; preserve official structure. -->`)
    .replace("</body>", "<!-- AUTHORITY_CONTENT_SLOT_END -->\n</body>");
}

function applyAuthorityContrast(source, colorSystem) {
  const contrastStyle = `    <style data-authority-contrast>
      :root, body, [data-composition-id] {
        --bg: var(--broadcast-field) !important;
        --fg: var(--broadcast-ink) !important;
        --surface: var(--broadcast-surface) !important;
        --border: var(--broadcast-line) !important;
        --muted: var(--broadcast-muted) !important;
        --brand: var(--broadcast-accent) !important;
        --accent: var(--broadcast-accent) !important;
        --accent-2: var(--broadcast-signal) !important;
        --mk-ink: var(--broadcast-ink) !important;
        --mk-ink-dim: var(--broadcast-muted) !important;
        --mk-ink-dark: var(--broadcast-ink) !important;
        --mk-ink-dim-dark: var(--broadcast-muted) !important;
        --mk-accent: var(--broadcast-accent) !important;
        --mk-accent-dark: var(--broadcast-line) !important;
      }
    </style>`;
...
function retokenAuthoritySource(source) {
  return source
    .replace(/html\s*,\s*body\s*\{[^}]*\}/g, "")
    .replaceAll("#f8fafc", "var(--broadcast-contrast-field)")
    .replaceAll("#0b0c0e", "var(--broadcast-field)")
    .replaceAll("#71f5a7", "var(--broadcast-accent)")
    .replaceAll("#61a8ff", "var(--broadcast-accent)")
    .replaceAll("#c5a3ff", "var(--broadcast-signal)")
    .replace(/#fff(?![0-9a-f])/gi, "var(--broadcast-contrast-field)");
}
...
function fuseOfficialComponent(html, source, officialId, compositionId, values, duration, sceneId) {
  ...
  let content = retokenAuthoritySource(extracted)
    .replace(/<script src="https?:\/\/[^\"]*gsap[^\"]*"><\/script>/g, "")
    .replaceAll("#root", "#authority-official-mount")
    .replaceAll('getElementById("root")', 'getElementById("authority-official-mount")')
    .replaceAll("getElementById('root')", "getElementById('authority-official-mount')");
  const mount = html.match(/<div[\s\S]*?id="authority-official-mount"[\s\S]*?>\s*<\/div>/)?.[0];
  ...
  let fused = html
    ...
    .replace("AUTHORITY_CONTENT_SLOT_BEGIN", "AUTHORITY_BEHAVIOR_BEGIN")
    .replace("AUTHORITY_CONTENT_SLOT_END", "AUTHORITY_BEHAVIOR_END")
...
function applyPresenterGeometry(html, presenter) {
  ...
  return html.replace('id="authority-official-mount"', `id="authority-official-mount" style="${geometry}"`);
}
...
function main(argv = process.argv.slice(2)) {
  ...
  const readStarter = name => {
    ...
    if (!fs.existsSync(starterPath)) {
      throw new Error(`Missing staged starter: ${path.relative(projectRoot, starterPath)}. Run the Authority authoring-kit staging command first.`);
    }
    return fs.readFileSync(starterPath, "utf8");
  };
  const plainStarter = readStarter("authority-scene.html");
  const plainMotion = readStarter("authority-scene.motion.json");
  const needsPresenterStarter = args.presenterMode === "on" && args.presenterScenes.length > 0;
  const presenterStarter = needsPresenterStarter ? readStarter("authority-presenter-scene.html") : null;
  const presenterMotion = needsPresenterStarter ? readStarter("authority-presenter-scene.motion.json") : null;
  ...
  for (const scene of scenes) {
    ...
    const layoutRoot = path.join(projectRoot, ".style-reference/authority-broadcast/layouts/source");
    ...
    const sourceId = layoutId
      ...
      : presenter ? "authority-presenter-scene" : "authority-scene";
    ...
    if (layoutId && registryCode) {
      const sourceMatch = html.match(/data-authority-content-slot="official"[\s\S]*?data-composition-id="([^"]+)"[\s\S]*?data-composition-src="([^"]+)"/);
      ...
      if (registryCode === "B") {
        ...
        isolatedSource = normalizeOfficialEnvelope(applyAuthorityContrast(markBlockContent(isolatedSource, scene.id), args.colorSystem), scene.duration);
        fs.writeFileSync(isolatedPath, isolatedSource);
        const originalMount = html.match(/<div[\s\S]*?<\/div>/)?.[0] || "";
        html = html
          .replace(/<!-- AUTHORITY_CONTENT_SLOT_BEGIN:[\s\S]*?<!-- AUTHORITY_CONTENT_SLOT_END -->/, `<!-- AUTHORITY_OFFICIAL_SOURCE: edit the protected content slot in ${isolatedRelative}. -->\n      ${originalMount}`)
  ...
  const contentFontLink = fs.existsSync(path.join(projectRoot, "assets/authority-broadcast/content-font.css"))
...
${contentFontLink}    <style>
  ...
  <body>
    <div id="root" data-composition-id="${args.hostId}" data-color-system="${args.colorSystem}" data-width="1920" data-height="1080" data-duration="${cursor}" data-video-presenter="${args.presenterMode}" data-authority-timing="${provisionalTiming ? "provisional" : "final"}">
${mountHtml}
    </div>
    <script>
...
`;
  ...
  console.log(`Authority scene contract created: ${scenes.length} scene(s), ${cursor}s, presenter ${args.presenterMode}, global color system ${args.colorSystem}.`);
  if (provisionalTiming) console.log("Scene HTML is ready for content authoring; run authority-finalize-timing.mjs with measured durations before review.");
  console.log(layoutMap.size
    ? "Executable official starters are active; edit only AUTHORITY_CONTENT_SLOT values in fused component scenes or scene-specific block files under compositions/official/."
...
try {
  ...
  console.error(`Authority scaffold failed: ${error.message}`);
  process.exitCode = 1;
}
