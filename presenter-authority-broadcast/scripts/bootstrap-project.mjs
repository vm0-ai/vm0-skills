...
function parseArgs(argv) {
  const args = { presenterScenes: [], force: false, prepareOnly: false, mediaMode: "none", colorSystem: "authority-broadcast" };
...
function main() {
  ...
  const existingFontCss = path.join(projectRoot, "assets/authority-broadcast/content-font.css");
  ...
  run(process.execPath, stageArgs, projectRoot, "Authority stage/install");

  if (args.prepareOnly) {
    console.log(`Authority media-parallel preparation complete: ${projectRoot}`);
    console.log("After media finishes, measure exact durations and run scripts/authority-scaffold-scenes.mjs once.");
    return;
  }

  const scaffoldArgs = [
    path.join(projectRoot, "scripts/authority-scaffold-scenes.mjs"),
    "--project", projectRoot,
    "--host-id", args.hostId,
    "--presenter", args.presenter,
    "--language", args.language,
  ...
  run(process.execPath, scaffoldArgs, projectRoot, "Authority scene scaffold");
  console.log(`Authority bootstrap complete: ${projectRoot}`);
}

try {
  main();
} catch (error) {
  console.error(`Authority bootstrap failed: ${error.message}`);
  process.exitCode = 1;
}
