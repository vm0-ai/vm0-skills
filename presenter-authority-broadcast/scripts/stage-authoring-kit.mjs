...
function usage(exitCode = 0) {
  ...
  out([
    ...
    "",
    "Copies the Authority Broadcast integration kit and selected executable official-component starters into an existing HyperFrames project.",
    ...
    "  --color-tokens <file>  Optional CSS collection defining semantic broadcast tokens for data-color-system=custom",
...
function parseArgs(argv) {
  const args = { layoutIds: [], archetypeIds: [], install: false, cleanManaged: false, force: false, language: "en", mediaMode: "none", colorSystem: "authority-broadcast" };
...
const existingContentFontCss = path.join(projectRoot, "assets/authority-broadcast/content-font.css");
...
const referenceRoot = ".style-reference/authority-broadcast";
...
function cleanManagedReferences(referenceRoot) {
  ...
  if (args.presenterMode === "off") {
    ...
    for (const relative of [
      "assets/authority-broadcast/presenters/p1.png",
      "assets/authority-broadcast/presenters/p2.png",
      "compositions/authority-presenter-scene.html",
      "compositions/authority-presenter-scene.motion.json",
...
function writeContentFont() {
  if (!args.contentFont) {
    if (!fs.existsSync(existingContentFontCss)) {
      writeReference("assets/authority-broadcast/content-font.css", [
        ".authority-frame {",
        "  --font-body: var(--authority-font-body);",
        "  --font-display: var(--authority-font-display);",
        "  --font-mono: var(--authority-font-mono);",
        "}",
        "",
      ].join("\n"));
    }
  ...
  const fontDestination = `assets/authority-broadcast/fonts/content.${extension}`;
  for (const candidate of ["woff2", "woff", "ttf", "otf", "ttc"]) {
    if (candidate !== extension) removeManaged(`assets/authority-broadcast/fonts/content.${candidate}`);
  }
  ...
  const contentFontCss = [
    ...
    "",
    ".authority-frame {",
    '  --authority-font-body: "AuthorityContent", AuthorityBody, Arial, sans-serif;',
    '  --authority-font-display: "AuthorityContent", AuthorityBody, Arial, sans-serif;',
    '  --authority-font-editorial: "AuthorityContent", AuthorityEditorial, Georgia, serif;',
    '  --authority-font-mono: "AuthorityContent", AuthorityMono, monospace;',
    "  --font-body: var(--authority-font-body);",
    "  --font-display: var(--authority-font-display);",
    "  --font-mono: var(--authority-font-mono);",
    "}",
  ...
  copied.push("assets/authority-broadcast/content-font.css");
}

function buildSelectionReference() {
  const lines = [
    "# Authority Broadcast Selected Reference",
    "",
    ...
    "- Keep the Authority background and presenter settled; use the mapped item's semantic content motion.",
...
for (const font of ["body.woff2", "editorial.woff2", "condensed.woff2", "mono.woff2"]) {
  copy(`assets/fonts/${font}`, `assets/authority-broadcast/fonts/${font}`);
}
if (args.cleanManaged) cleanManagedReferences(referenceRoot);

copy("assets/runtime/gsap.min.js", "assets/runtime/gsap.min.js");
copy("assets/runtime/authority.css", "assets/authority-broadcast/authority.css");
copy("assets/runtime/color-system.css", "assets/authority-broadcast/color-system.css");
if (args.colorTokens) copyExternal(args.colorTokens, "assets/authority-broadcast/custom-color-system.css");
else copy("assets/runtime/custom-color-system.css", "assets/authority-broadcast/custom-color-system.css");
writeColorSystemReceipt();
writeContentFont();
copy("scripts/review-project.mjs", "scripts/authority-review.mjs");
copy("scripts/scaffold-scenes.mjs", "scripts/authority-scaffold-scenes.mjs");
copy("scripts/finalize-timing.mjs", "scripts/authority-finalize-timing.mjs");
copy("scripts/set-color-system.mjs", "scripts/authority-set-color-system.mjs");
copy("assets/starter/compositions/authority-scene.html", "compositions/authority-scene.html");
copy("assets/starter/compositions/authority-scene.motion.json", "compositions/authority-scene.motion.json");
if (args.presenterMode === "on") {
  copy("assets/presenters/p1.png", "assets/authority-broadcast/presenters/p1.png");
  copy("assets/presenters/p2.png", "assets/authority-broadcast/presenters/p2.png");
  copy("assets/starter/compositions/authority-presenter-scene.html", "compositions/authority-presenter-scene.html");
  copy("assets/starter/compositions/authority-presenter-scene.motion.json", "compositions/authority-presenter-scene.motion.json");
}
...
if (adapterLayouts.length) {
  copy("assets/runtime/authority-layout-adapter.css", "assets/runtime/authority-layout-adapter.css");
  for (const id of adapterLayouts) {
    const filename = layoutFilename(id);
    copy(`assets/layouts/adapters/${filename}.html`, `compositions/authority-adapters/${filename}.html`);
  }
}

if (adapterLayouts.some(id => id.startsWith("media/"))) {
  copy("assets/style-master-a.png", "assets/authority-broadcast/media/style-master-a.png");
  copy("assets/style-master-b.png", "assets/authority-broadcast/media/style-master-b.png");
  copy("assets/presenters/p1.png", "assets/authority-broadcast/media/presenter.png");
}

if (args.layoutIds.some(id => routerBinding(id).routerCode === "M")) {
  copy("assets/layouts/adapters/authority-text-evidence.html", "compositions/authority-text-evidence.html");
}
...
console.log(`Authority Broadcast kit staged in ${projectRoot}`);
...
if (!args.install) console.log("Open .style-reference/authority-broadcast/SELECTION.md and install each mapped Registry item directly, or rerun with --install.");
...
console.log("Use authority-scaffold-scenes.mjs with --layout-map so IDs, durations, official mounts, and motion sidecars remain aligned.");
console.log("Finish all settled static scenes, then use scripts/authority-review.mjs for preflight, static, preview, and release review.");
