const groups = [
  ["Orientation and closure", [
    ["orientation/authority-cover", "Authority Cover", "titlecard-lockup"],
...
for (const [groupName, items] of groups) {
  ...
  for (const [id, label, registryItem] of items) {
    ...
    article.innerHTML = `<a class="preview" data-preview-stem="${stem}" href="preview/authority-broadcast/${stem}.png"><img src="preview/authority-broadcast/${stem}.png" alt="${label} official HyperFrames render proof"></a><div class="copy"><h2>${label}</h2><p>${id}</p><span>HyperFrames · ${registryItem}</span><a href="source/${stem}.html">Open executable starter</a></div>`;
    grid.append(article);
  }
...
function applyPalette(name, updateUrl = true) {
  const palette = palettes[name] || palettes["authority-broadcast"];
  const selected = palettes[name] ? name : "authority-broadcast";
