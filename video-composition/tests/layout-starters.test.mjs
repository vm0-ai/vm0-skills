...
test("all forty Router layouts have executable starters and motion contracts", () => {
  ...
  for (const id of ids) {
    ...
    assert.match(html, /CONTENT_SLOT_BEGIN/);
    assert.match(html, /<main id="root"/);
    assert.match(html, /data-vc-content-slot="official"/);
    ...
    const adapterMatch = html.match(/data-composition-src="compositions\/vc-adapters\/([^"/]+\.html)"/);
    if (adapterMatch) {
      const adapter = fs.readFileSync(path.join(adapterRoot, adapterMatch[1]), "utf8");
      assert.match(adapter, /data-vc-content-ready="true"/);
      assert.match(adapter, /assets\/runtime\/vc-layout-adapter\.css/);
      assert.match(adapter, /window\.__timelines\[stage\.dataset\.timelineKey\]/);
      assert.doesNotMatch(adapter, /<script[^>]+src="assets\/runtime\/vc-layout-adapter\.js"/);
...
test("short-prone gallery starters retain explanatory working density", () => {
  for (const id of [
    "orientation/headline-cover", "orientation/section-pivot",
