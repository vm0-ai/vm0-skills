...
const defaultProject = path.resolve(skillRoot, "../../../video-composition-layouts");
const projectRoot = path.resolve(process.argv[2] || defaultProject);
const outputRoot = path.join(skillRoot, "assets/layouts/adapters");
const adapterRuntime = fs.readFileSync(path.join(skillRoot, "assets/runtime/vc-layout-adapter.js"), "utf8").trim();
...
const specs = [
  ...
  { id: "time/handoff-flow", kind: "process", origins: ["tracing-beam"], variables: processVars("Responsibility moves with a named artifact", "HANDOFF CONTROL", "01|Policy team|Signed decision → region;02|Regional lead|Readiness return → service;03|Service owner|Published guidance → public;04|Review team|Outcome record → composition", "FINAL STATE · ACCOUNTABLE DELIVERY"), markup: processLayout() },
  ...
  { id: "system/decision-flow", kind: "process", origins: ["tracing-beam", "state-chip-rail"], variables: processVars("The decision path includes a real exception", "DECISION CONTROL", "01|Assess evidence|Is the record complete?;YES|Approve|Confirm composition and threshold;NO|Return|Name the missing evidence;FINAL|Publish|Record decision and route", "OUTCOME · EVERY BRANCH HAS AN OWNER"), markup: processLayout() },
  { id: "media/media-left", kind: "media", mediaSide: "left", origins: ["particle-image-reveal", "line-by-line-slide"], variables: mediaVars("EVIDENCE IMAGE", "The source image establishes the decision context", "assets/video-composition/media/style-master-a.png", "Approved this visual system reference frame", "Deadline remains visible;Presenter and content stay disjoint", "STYLE MASTER · APPROVED REFERENCE"), markup: media() },
  { id: "media/media-right", kind: "media", mediaSide: "right", origins: ["screen-flow-carousel", "line-by-line-slide"], variables: mediaVars("DOCUMENT READING", "The record and the interpretation remain side by side", "assets/video-composition/media/style-master-b.png", "Implementation record · page 12", "Measure passes;Phase one begins with large institutions", "OFFICIAL RECORD · VERIFIED EXCERPT"), markup: media() },
  { id: "media/evidence-grid", kind: "media-grid", origins: ["screen-flow-carousel", "grid-card-assemble"], variables: [stringVar("kicker", "Kicker", "EVIDENCE SET"), stringVar("title", "Evidence statement", "Three records support the same conclusion"), stringVar("summary", "Interpretation", "Each item has a visible role, source label and source-backed image."), stringVar("image_1", "Primary image", "assets/video-composition/media/style-master-a.png"), stringVar("caption_1", "Primary caption", "Approved presenter-led reference"), stringVar("image_2", "Second image", "assets/video-composition/media/style-master-b.png"), stringVar("caption_2", "Second caption", "Decision and deadline structure"), stringVar("image_3", "Third image", "assets/video-composition/media/presenter.png"), stringVar("caption_3", "Third caption", "Bundled identity reference"), stringVar("source", "Source", "AUTHORITY EVIDENCE SET · THREE ITEMS")], markup: mediaGrid() },
  { id: "media/evidence-hero", kind: "media", mediaSide: "left", origins: ["particle-image-reveal", "inline-highlight"], variables: mediaVars("PRIMARY EVIDENCE", "One record carries the decisive proof", "assets/video-composition/media/style-master-b.png", "Official implementation record", "The effective date is explicit;The rollout condition is traceable", "OFFICIAL RECORD · PAGE 12"), markup: media() },
];
...
function media() { return `<div class="ab-frame"><section class="ab-media-layout"><figure class="ab-media-card" data-motion="media"><img src="assets/video-composition/media/style-master-a.png" data-media="image" alt="" /></figure><div class="ab-media-copy">${header()}<ul class="ab-callouts" data-list="callouts"></ul>${footer()}</div></section></div>`; }
function mediaGrid() { return `<div class="ab-frame">${header()}<section class="ab-media-grid"><figure data-motion="media"><img src="assets/video-composition/media/style-master-a.png" data-media="image_1" alt="" /><figcaption data-bind="caption_1"></figcaption></figure><figure data-motion="media"><img src="assets/video-composition/media/style-master-b.png" data-media="image_2" alt="" /><figcaption data-bind="caption_2"></figcaption></figure><figure data-motion="media"><img src="assets/video-composition/media/presenter.png" data-media="image_3" alt="" /><figcaption data-bind="caption_3"></figcaption></figure></section>${footer()}</div>`; }

function adapterHtml(spec, duration) {
  const stem = spec.id.replace("/", "--");
  const compositionId = `vc-adapter-${stem}`;
  const variables = JSON.stringify(spec.variables).replaceAll("'", "&#39;");
  return `<!doctype html>
<!-- Content-ready content adapter. Official HyperFrames behavior origins: ${spec.origins.join(" + ")}. -->
<html lang="en" data-composition-id="${compositionId}" data-composition-duration="${duration}" data-vc-content-ready="true" data-registry-origin="${spec.origins.join(" + ")}" data-composition-variables='${variables}'>
  <head><meta charset="UTF-8" /></head>
  <body>
    <template>
      <div id="root" data-composition-id="${compositionId}" data-duration="${duration}" data-width="1920" data-height="1080" data-fps="30">
        <style>@import url("assets/runtime/vc-layout-adapter.css");</style>
        <section id="${stem}-stage" class="vc-adapter-stage clip" data-vc-adapter-stage data-kind="${spec.kind}"${spec.mediaSide ? ` data-media-side="${spec.mediaSide}"` : ""} data-timeline-key="${compositionId}" data-start="0" data-duration="${duration}" data-track-index="0">
          ${spec.markup}
        </section>
        <script>
${adapterRuntime}
...
for (const spec of specs) {
  ...
  const replacement = match[0]
    ...
    .replace(/data-composition-src="[^"]+"/, `data-composition-src="compositions/vc-adapters/${stem}.html"`)
...
const projectAdapterRoot = path.join(projectRoot, "compositions/vc-adapters");
...
for (const name of ["vc-layout-adapter.css", "vc-layout-adapter.js"]) {
  fs.copyFileSync(path.join(skillRoot, "assets/runtime", name), path.join(projectRoot, "assets/runtime", name));
}
const mediaRoot = path.join(projectRoot, "assets/video-composition/media");
...
console.log(`Built and wired ${specs.length} content-ready content adapters.`);
