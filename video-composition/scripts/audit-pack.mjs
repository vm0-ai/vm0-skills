...
const required = [
  ...
  "assets/runtime/stage.css", "assets/runtime/vc-layout-adapter.css", "assets/runtime/vc-layout-adapter.js", "assets/runtime/gsap.min.js",
  "assets/starter/compositions/vc-scene.html", "assets/starter/compositions/vc-scene.motion.json",
  "assets/starter/compositions/vc-presenter-scene.html", "assets/starter/compositions/vc-presenter-scene.motion.json", "assets/starter/demo-index.html",
...
if (!errors.length) {
  ...
  const runtimeCss = read("assets/runtime/stage.css");
  const colorSystemCss = read("assets/runtime/color-system.css");
  const adapterCss = read("assets/runtime/vc-layout-adapter.css");
  const adapterJs = read("assets/runtime/vc-layout-adapter.js");
  ...
  const starter = read("assets/starter/compositions/vc-scene.html");
  const presenterStarter = read("assets/starter/compositions/vc-presenter-scene.html");
  const demoIndex = read("assets/starter/demo-index.html");
  const starterMotion = JSON.parse(read("assets/starter/compositions/vc-scene.motion.json"));
  const presenterStarterMotion = JSON.parse(read("assets/starter/compositions/vc-presenter-scene.motion.json"));
  ...
  if (template.id !== "navy-cobalt" || template.kind !== "open-visual-style") errors.push("invalid template identity");
  ...
  if (template.registryIntegrationPolicy?.receipt !== ".style-reference/video-composition/REGISTRY-INSTALLS.json" || template.registryIntegrationPolicy?.component !== "fuse-official-source-into-scene-and-bake-starter-defaults" || template.registryIntegrationPolicy?.block !== "copy-official-source-per-scene-and-edit-marked-content" || template.registryIntegrationPolicy?.guessing !== "forbidden") errors.push("Registry integration receipt policy is invalid");
  if (template.fontPolicy?.nonLatin !== "stage-user-supplied-licensed-content-font" || template.fontPolicy?.runtimeToken !== "VcContent") errors.push("content font policy is invalid");
  ...
  if (template.assets?.sceneStarter !== "assets/starter/compositions/vc-scene.html" || template.assets?.presenterSceneStarter !== "assets/starter/compositions/vc-presenter-scene.html" || template.assets?.presenterHeadSceneStarter) errors.push("complete-media starter asset paths are invalid");
  if (fs.existsSync(absolute("assets/starter/compositions/vc-presenter-head-scene.html")) || fs.existsSync(absolute("assets/starter/compositions/vc-presenter-head-scene.motion.json"))) errors.push("retired standalone head presenter starter remains published");
  ...
  if (template.workflowPolicy?.ownerWhenExplicitlySelected !== "presenter-navy-cobalt" || template.workflowPolicy?.skipGenericWorkflow !== "general-video" || template.workflowPolicy?.inlineStarterSceneLimit !== 12) errors.push("specialized fast-workflow ownership is invalid");
  ...
  for (const id of layoutIds) {
    ...
    if (!layoutSource.includes(`data-layout-id="${id}"`) || !layoutSource.includes("CONTENT_SLOT_BEGIN") || !layoutSource.includes("CONTENT_SLOT_END")) errors.push(`${id}: executable starter lacks its protected content slot`);
    ...
    const adapterMatch = layoutSource.match(/data-composition-src="compositions\/vc-adapters\/([^"/]+\.html)"/);
    if (adapterMatch) {
      const adapter = read(`assets/layouts/adapters/${adapterMatch[1]}`);
      if (!adapter.includes('data-vc-content-ready="true"') || !adapter.includes("assets/runtime/vc-layout-adapter.css") || !adapter.includes("window.__timelines[stage.dataset.timelineKey]")) errors.push(`${id}: content adapter contract is incomplete`);
      if ((adapter.match(/"role":"content"/g) || []).length < 4) errors.push(`${id}: content adapter does not expose sufficient semantic content fields`);
      if (/placeholder|skeleton|baw-default|browser chrome/i.test(adapter)) errors.push(`${id}: content adapter contains demo placeholders`);
    }
  ...
  if (!scaffoldScript.includes("fuseOfficialComponent") || !scaffoldScript.includes("bakeVariableDefaults") || !scaffoldScript.includes("markBlockContent") || !scaffoldScript.includes("applyStageContrast") || !scaffoldScript.includes("compositions/official/")) errors.push("scene scaffolder does not fuse components, isolate blocks, and enforce contrast");
  if (!stageScript.includes("--content-font") || !stageScript.includes("VcContent") || !stageScript.includes('copy("scripts/scaffold-scenes.mjs", "scripts/scaffold-scenes.mjs")') || !stageScript.includes('copy("scripts/finalize-timing.mjs", "scripts/finalize-timing.mjs")')) errors.push("staging script does not install content font and scene timing support");
  if (!scaffoldScript.includes("index.motion.json") || !scaffoldScript.includes("presenterScenes") || !scaffoldScript.includes("layoutMap") || !scaffoldScript.includes("CONTENT_SLOT") || !scaffoldScript.includes("Refusing to overwrite")) errors.push("scene scaffolder does not preserve the layout-bound safe contract");
  if (!scaffoldScript.includes('durationToken === "auto"') || !scaffoldScript.includes("data-vc-timing") || !timingFinalizer.includes("Authored scene content was preserved") || !timingFinalizer.includes("data-vc-timing") || !timingFinalizer.includes("compositions\\/official")) errors.push("provisional scene timing cannot be finalized safely without rebuilding authored content");
  if (!stageScript.includes('copy("scripts/review-project.mjs", "scripts/review-project.mjs")') || !stageScript.includes("--clean-managed") || !stageScript.includes("cleanManagedReferences")) errors.push("staging script does not install the review tool and controlled cleanup");
  if (!authoring.includes("scripts/review-project.mjs") || !motion.includes("scripts/review-project.mjs") || !skill.includes("scripts/review-project.mjs")) errors.push("documentation does not route review through one deterministic entrypoint");
  ...
  if (!starterBuilder.includes("CONTENT_SLOT_BEGIN") || !starterBuilder.includes("data-registry-code") || !starterBuilder.includes("Expected 40 rendered layouts")) errors.push("layout starter builder does not preserve bounded official contracts");
  if (!adapterBuilder.includes("data-vc-content-ready") || !adapterBuilder.includes("vc-layout-adapter.css") || !adapterBuilder.includes("vc-layout-adapter.js")) errors.push("content adapter builder contract is incomplete");
  if (!stageScript.includes("assets/runtime/vc-layout-adapter.css") || !stageScript.includes("compositions/vc-adapters/") || !stageScript.includes("assets/video-composition/media/")) errors.push("staging does not copy selected content adapters and media dependencies");
  ...
  if (!runtimeCss.includes(".vc-content-stage") || !runtimeCss.includes("place-items: center") || !runtimeCss.includes(".vc-official-mount")) errors.push("runtime CSS does not provide centered and full-canvas content hosts");
  if (!runtimeCss.includes("--stage-font-body") || !runtimeCss.includes("--stage-font-display") || !runtimeCss.includes("--stage-font-mono")) errors.push("runtime CSS does not expose overridable content font tokens");
  if (!runtimeCss.includes('[data-video-presenter="off"] .presenter-slot') || !runtimeCss.includes('.vc-frame[data-presenter="off"]') || !runtimeCss.includes('.vc-frame[data-title-chrome="off"]')) errors.push("runtime CSS does not support structurally omitted presenter and title chrome");
  ...
  if (!starter.includes('data-composition-id="vc-scene"') || !starter.includes('window.__timelines["vc-scene"] = tl')) errors.push("starter composition ids do not match");
  if (!starter.includes('data-presenter="off"') || !starter.includes('data-title-chrome="off"') || !starter.includes("vc-content-stage") || !starter.includes("line-by-line-slide")) errors.push("starter must demonstrate centered official content with presenter and title chrome omitted");
  if (starter.includes("presenter-slot") || starter.includes("vc-public-rail")) errors.push("starter must not reserve presenter or rail space by default");
  if (!Array.isArray(starterMotion.assertions) || !starterMotion.assertions.length) errors.push("starter motion sidecar is empty");
  if (!presenterStarter.includes('data-composition-id="vc-presenter-scene"') || !presenterStarter.includes('window.__timelines["vc-presenter-scene"] = tl')) errors.push("presenter starter composition ids do not match");
  ...
  if (!demoIndex.includes('data-composition-src="compositions/vc-scene.html"') || !demoIndex.includes('data-video-presenter="off"')) errors.push("host wiring example does not demonstrate explicit presenter-off mode");
...
console.log("OK video-composition: 40 executable Registry starters, one-command bootstrap, linting Preflight, automatic scene-delta Preview, portable content fonts, and <=2100-word default context");
