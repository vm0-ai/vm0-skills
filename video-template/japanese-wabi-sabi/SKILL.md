...
> The reference still holds the wabi-sabi look on a different subject (teacup vs. alley) — the style is subject-invariant. For image-capable modes, use a user-approved frame of the brief's subject with its verified input role; text-only previews guide the prompt only.
