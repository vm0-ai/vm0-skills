...
> The reference still holds the sports performance ad look on a different sport (boxing vs. running) - the style is sport-invariant and product-aware. For image-capable modes, use a user-approved frame of the brief's subject with its verified input role; text-only previews guide the prompt only.
