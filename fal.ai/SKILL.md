...
curl -s -X POST "https://fal.run/fal-ai/flux/schnell" \
  --header "Authorization: Key ${FAL_KEY}" \
  --header "Content-Type: application/json" \
  -d '{"prompt": "A cute cat eating a cookie"}' \
  | jq -r '.images[0].url'
```
...
```bash
curl -s -X POST "https://fal.run/fal-ai/recraft-v3" \
  --header "Authorization: Key ${FAL_KEY}" \
  --header "Content-Type: application/json" \
  -d '{"prompt": "Abstract art, vibrant colors"}' \
  | jq -r '.images[0].url'
```
...
curl -s -X POST "https://fal.run/fal-ai/nano-banana-pro" \
  --header "Authorization: Key ${FAL_KEY}" \
  --header "Content-Type: application/json" \
  -d '{"prompt": "Mountain landscape", "image_size": "landscape_16_9"}' \
  | jq -r '.images[0].url'
```
...
curl -s -X POST "https://fal.run/fal-ai/nano-banana-pro" \
  --header "Authorization: Key ${FAL_KEY}" \
  --header "Content-Type: application/json" \
  -d '{"prompt": "A minimalist workspace"}' \
