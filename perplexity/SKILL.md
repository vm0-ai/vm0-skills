...
```bash
curl -s -X POST "https://api.perplexity.ai/chat/completions" --header "Authorization: Bearer $PERPLEXITY_API_KEY" --header "Content-Type: application/json" -d @/tmp/perplexity_request.json
```
...
```bash
curl -s -X POST "https://api.perplexity.ai/search" --header "Authorization: Bearer $PERPLEXITY_API_KEY" --header "Content-Type: application/json" -d @/tmp/perplexity_request.json
```
