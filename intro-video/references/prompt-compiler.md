...
- State one approximate target length and nothing else about duration. `no longer than 30 seconds` style caps were ignored in a real run (a 20-second target rendered 34.8 seconds); if the deliverable must stay under a limit, set the target well below it (about 18 seconds for a 30-second ceiling) or use verbatim mode, where length follows the script.
