...
Presenter composition depends on the look type. HeyGen's own Video Agent guidance treats a `photo_avatar` as a presenter whose environment is generated together with the avatar, while a `studio_avatar` (a preset cutout) and any transparent preview only receive a best-effort BACKGROUND NOTE. The managed catalog listed only public `studio_avatar` looks as of 2026-09-08; three native runs on three styles placed such looks on a plain white stage, and two near-square looks were cropped at the head in landscape output despite the official notes. Output resolution has no parameter on this endpoint; all three runs returned 1280×720 at 25 fps, and the "default 1080p" statement in HeyGen's output specifications belongs to `/v3/videos`. Native pricing observed on the managed route was about 42 credits per output second.
