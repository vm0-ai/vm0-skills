...
Presenter composition depends on the look type. HeyGen's own Video Agent guidance treats a `photo_avatar` as a presenter whose environment is generated together with the avatar, while a `studio_avatar` (a preset cutout) and any transparent preview only receive a best-effort BACKGROUND NOTE. The managed catalog listed only public `studio_avatar` looks as of 2026-09-08. Such looks render as a cutout on a white stage with a cropped head when the prompt carries only the notes, and render with a full head inside a generated environment when the prompt also carries the compiler's presenter sentences and the script-freedom directive (validated 2026-09-07 and 2026-09-08 with the same look, style, and account type). Output resolution has no parameter on this endpoint and follows the HeyGen account: a paid Creator account rendered 1920×1080 on 2026-09-07, while the wallet account used on 2026-09-08 rendered 1280×720 at 25 fps for every run; the "default 1080p" statement in HeyGen's output specifications belongs to `/v3/videos`. Native pricing observed on the managed route was about 42 credits per output second.
