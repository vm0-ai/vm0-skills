...
Video Agent always writes and voices narration. Attached MP3/WAV files are reference material; no field makes them the soundtrack, and nothing removes narration or audio from the render. `No voiceover`, `silent`, and `Original audio` are therefore Okou-orchestrated (controlled-route) outcomes, and so is `No avatar`: an omitted `avatar_id` means automatic selection, not omission, so only a composition that leaves the presenter layer out can guarantee no digital human on screen. Stripping the audio track from a finished native video is not equivalent: the pacing and captions were composed around narration, and the narrated seconds are still billed. A pasted script is treated as a concept unless the prompt says otherwise; verbatim narration can be requested but not guaranteed, so exact wording plus exact timing is also a controlled-route job.
...
Presenter framing on the direct video endpoint (`POST /v3/videos`) is decided by `fit`: `cover` fills the frame width and crops a near-square look at the head, `contain` keeps the whole look, and a landscape look with a baked-in environment keeps the head under either. The environment comes only from the look image; the engine (`avatar_iii` or `avatar_iv`) changes neither framing nor background, and the scenes endpoint omits `engine` for accepted `avatar_iii` requests. Video Agent has no fit field. Engines are priced differently upstream, but no decision here needs a rate: what matters is that the managed route bills only the Avatar III presenter path and the Video Agent path, so an Avatar IV request never buys anything on it.

Presenter composition depends on the look type and on the prompt. HeyGen's own Video Agent guidance treats a `photo_avatar` as a presenter whose environment is generated together with the avatar, while a `studio_avatar` (a preset cutout) and any transparent preview receive only prompt guidance. Observed on the managed route: a transparent studio look renders inside a generated environment with a full head when the prompt is the complete compiled skeleton, and as a cutout with a cropped head when the presenter sentences or the script-mode directive are missing or the prompt is long and scene-by-scene. Output resolution has no parameter on this endpoint and follows the HeyGen account plan: a paid Creator account rendered 1920×1080, a wallet account renders 1280×720 at 25 fps; the "default 1080p" statement in HeyGen's output specifications belongs to `/v3/videos`. The managed route is billed by output second, so a video that overshoots its target costs more than one that lands on it, and each submission is a fresh charge. What a job actually cost comes back on the job as `creditsCharged`; credit and plan failures go through `okou doctor credit`. A prompt's duration caps are ignored; only the approximate target is followed.
...
`POST /v3/hyperframes/renders` renders a packaged authored HTML project, with explicit fps/format/resolution/ratio controls. It does not create a narrative from a prompt. The preservation route requires the release containing the managed `okou video render` command and APIs. That route accepts an owned Okou ZIP, submits it with the platform’s server-side `HEYGEN_API_KEY`, and returns a persisted Okou artifact. The existing Intro Video switch gates new jobs; admitted jobs remain recoverable. The first release exposes 1080p/30 fps standard-quality MP4 in 16:9 or 9:16. It does not use `HEYGEN_TOKEN`, a personal connector or a direct `hyperframes cloud render` login. Local authoring flags and cloud output controls are not interchangeable. If the managed release is unavailable, report that dependency; do not silently substitute another account or export route.
