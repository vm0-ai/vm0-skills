...
A completed video also carries `captioned_video_url`, `subtitle_url` (SRT), `thumbnail_url`, and `gif_url` upstream; the managed route currently persists only the MP4. Exact scripts, original pages/frames/audio, avatar placement, and overlay geometry are not guaranteed. `mode: chat` is not a reliable user-review barrier because the agent may proceed to generation.
