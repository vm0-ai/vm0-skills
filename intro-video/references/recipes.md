...
- Use the arc as an order, not as labels, and never as timestamps. The native prompt always carries one flowing narration paragraph, at any length: scene labels and per-scene media notes are what send Video Agent back to templates, the cutout, and invented on-screen detail. Scene grouping belongs to the controlled route, where Okou lays out the timeline itself and each scene genuinely needs its own media mapping; for a long or data-heavy native video, let the arc order the sentences and leave the visuals to the production lines.
