...
- `environment`: decode the actual preview image. An alpha channel with transparent pixels, a solid color, or a visually empty field means no environment; only a real scene counts as `real`. Read it from the pixels; `preferredOrientation` and the type describe something else.
