...
Tell the user the route, its consequence, and the inferred duration and language in one sentence before generation. Do not add a review gate they did not request. Do not silently switch routes, identities, or fidelity levels after a failure.

## Accept or reject

A completed provider job is a QA candidate, not a deliverable. Apply [QA](references/qa.md): probe the media, inspect representative frames, transcribe when the brief fixes wording or language, and compare against the brief. A billed output that fails the gate is reported with its identifiers and unmet requirements; do not call it polished, do not automatically submit another paid job, and do not switch routes without the user's direction.
