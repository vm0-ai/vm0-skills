...
**Open five files before submitting, and no others:** this one, [brief](references/brief.md), [recipes](references/recipes.md), [prompt compiler](references/prompt-compiler.md), and the execution reference for the selected route. The rest have triggers: [catalogs](references/catalogs.md) only when a choice is delegated or an exact ID needs a compatibility check — with style, avatar and voice all supplied, query nothing; [input preparation](references/input-preparation.md) only with attachments; [QA](references/qa.md) only after a job returns; [provider boundaries](references/provider-boundaries.md) only for a capability the route does not cover. Reading the rest anyway costs the user minutes before anything is submitted.
...
Without an expansion directive, HeyGen pads a short script with silence to reach a stated target, so in verbatim mode the only length that appears is the script's own estimate.
...
- **Add the FRAMING NOTE only when the look is also narrow**: for landscape output a catalog `imageWidth` under 1.20 times its `imageHeight`; the mirror rule for portrait. A wide look needs no FRAMING NOTE and still needs the directive above. The two properties are independent, and `avatar_type`, `preferredOrientation`, and width never establish an environment.
...
The engine (Avatar III or Avatar IV) changes neither framing nor background, so the look and the prompt are the whole lever. Do this for every native submission with a presenter:
...
3. **Follow it with the presenter adaptation directive**, verbatim from the compiler, for every `studio_avatar`, `digital_twin`, or transparent look: it tells the agent to create an AI-extended 16:9 (or 9:16) version of the selected presenter with the full head, hair, and shoulders inside the image and a complementary environment, wait for it, use that extended presenter in every presenter scene, and fit the presenter inside the frame instead of filling the width with the cutout. Omit it only for a look that already is a landscape (or portrait) image with a real environment. The engine is the provider's choice; it changes neither framing nor background.
4. **Put the narration in one quoted `Narration:` paragraph**, at any length, with the visuals carried by the production lines.
...
Tell the user the route, its consequence, the inferred duration and language, and any presenter or resolution capability gap in one sentence before generation. Add a review gate only when they ask for one. After a failure, bring the route, identity, or fidelity question back to the user before changing any of them.

## Accept or reject

A completed provider job is already the user's file. Deliver it first, with any defect you know of named in one sentence, then verify: checking before handing over puts minutes of Okou's own work in front of a video the user has already paid for. The prompt is the control and QA is the verification: probe the media, inspect representative frames, transcribe when the brief fixes wording, language, or brand names, and compare against the brief per [QA](references/qa.md). A defect the prompt could have prevented is a prompt error to fix before any retry; a defect that survived a complete prompt is a provider gap to disclose. In both cases the next paid job waits for the user's direction, and any retry carries a changed prompt.

QA is internal. The message delivers the video with its permanent URL and one line on the route, duration, and language; it never includes QA results: no check list, pass/fail table, tier labels, frame lists, transcripts, or evidence links. Keep the evidence in the workspace. Mention a defect when it changes what the user receives or what they should expect from the file they now have: a provider gap in one plain sentence with the alternative, a rejected output in one sentence with what you propose.
