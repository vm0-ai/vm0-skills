...
The head gets cropped when the user picks a look and the prompt says nothing about framing: Video Agent fits the raw cutout to the frame width, and a look narrower than the output loses the top of the head. The same look fitted inside the frame keeps it, and so does a look already about as wide as the output. There is no fit field to set, so the whole fix is the look plus the prompt, and it is two moves:

- **Give the agent a wider starting image.** When Okou chooses the look, take the widest low-crop one available (landscape output: decoded preview at least 1.20 times wider than tall; the mirror rule for portrait). This alone removes the crop.
- **Tell the agent to extend the look and fit it inside the frame**, with the adaptation directive, whenever the look has no real environment baked into its preview. It is what turns a transparent cutout into a landscape presenter with a background, and it is required even for a wide look, because width does not supply a background.

Those two are independent: a wide look still has no background, and a look with a real environment can still be the wrong shape. Decide each from the decoded preview — `avatar_type`, `preferredOrientation`, and the look's width never establish an environment. The engine (Avatar III or Avatar IV) changes neither, so never trade a look choice for an engine request. While the managed catalog offers only transparent `studio_avatar` looks, the normal case is wide enough but with no background: no FRAMING NOTE, but the adaptation directive and the BACKGROUND NOTE always. Do this for every native submission with a presenter:

1. **Resolve the look before writing anything.** Apply the two moves above, keep an explicitly chosen look even when it is near-square, and say so in the pre-generation sentence — a near-square or transparent look is the one the agent fits to the width when nothing tells it otherwise.
