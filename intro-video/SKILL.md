...
- **Voice:** an explicitly selected voice ID is exact. `Default` with a presenter prefers that look's actual default voice; if missing or unavailable for the selected route, automatically choose a compatible public voice in the narration language and tell the user about the substitution. Follow [voice resolution](references/catalogs.md#resolve-the-voice) and retain whether the user chose `Default` or an exact voice. A delegated voice means a public voice matching the narration language. `No voiceover` and `Original audio` are controlled-route requirements, never a muted native job.
