...
```bash
curl -s "https://api.zeptomail.com/v1.1/email" \
  -X POST \
  --header "Authorization: Zoho-enczapikey ${ZEPTOMAIL_API_KEY}" \
...
```bash
curl -s "https://api.zeptomail.com/v1.1/email/template" \
  -X POST \
  --header "Authorization: Zoho-enczapikey ${ZEPTOMAIL_API_KEY}" \
  --header "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "https://api.zeptomail.com/v1.1/email/batch" \
  -X POST \
  --header "Authorization: Zoho-enczapikey ${ZEPTOMAIL_API_KEY}" \
  --header "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "https://api.zeptomail.com/v1.1/email/template/batch" \
  -X POST \
  --header "Authorization: Zoho-enczapikey ${ZEPTOMAIL_API_KEY}" \
  --header "Content-Type: application/json" \
  -d '{
